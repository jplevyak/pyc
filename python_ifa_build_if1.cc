...
static void build_import_if1(char *sym, char *as, char *from, PycCompiler &ctx) {
  ...
  if (!m->built_if1) {
    Code **c;
    // Get code pointer for the current statement (either pyda or CPython path)
    if (pydmap.get((PyDAST *)ctx.node))
      c = &getAST((PyDAST *)ctx.node, ctx)->code;
    else
      c = &getAST((stmt_ty)ctx.node, ctx)->code;
    build_module_attributes_if1(m, *m->ctx, c);
    if (m->pymod && !m->is_builtin)
...
// Evaluate a string literal using Python's string parsing
static Sym *eval_string_pyda(PyDAST *n, PycCompiler &ctx) {
  // n->str_val is the raw string including quotes (e.g. "hello" or 'world' or """...""")
  // Use PyRun_String to evaluate the literal
  PyObject *result = PyRun_String(n->str_val, Py_eval_input, PyEval_GetBuiltins(), PyEval_GetLocals());
  if (!result) {
    PyErr_Clear();
    // Fallback: strip outer quotes naively
    cchar *s = n->str_val;
    int len = strlen(s);
    if (len >= 2) {
  ...
  if (PyString_Check(result))
    sym = make_string(PyString_AsString(result));
  else if (PyUnicode_Check(result)) {
    PyObject *encoded = PyUnicode_AsUTF8String(result);
    if (encoded) {
      sym = make_string(PyString_AsString(encoded));
      Py_DECREF(encoded);
    }
  }
  Py_DECREF(result);
  if (!sym) sym = make_string("");
  return sym;
}
...
static int build_builtin_call_pyda(PycAST *atom_ast, PyDAST *call_trailer, PycAST *ast, PycCompiler &ctx) {
  ...
  if (f == sym_super) {
    ...
    } else if (fun_ast->xstmt)
      get_syms_args(fun_ast, fun_ast->xstmt->v.FunctionDef.args, fun_has, ctx);
  ...
  switch (n->kind) {
    ...
    case PY_decorated: {
      ...
      if (def->kind == PY_funcdef) {
        ...
        gen_class_pyda(def, def_ast, ctx);
        exit_scope(ctx);
        ast->rval = def_ast->rval;
        ast->code = def_ast->code;
      }
    ...
    case PY_bool_not: {
      ...
      if1_gen(if1, &ast->code, v->code);
      ast->rval = new_sym(ast);
      if1_send(if1, &ast->code, 2, 1, map_unary_operator(Not), v->rval, ast->rval)->ast = ast;
      return 0;
    }
