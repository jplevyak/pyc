...
## What is still wrong, and the real fix

47 compile failures under the flag is a **precision** problem, not a
...
The real fix is the one this issue's title states: **arity participates
in CreationSet identity for record-able containers.** `[]` and `[2, 3]`
should get different CreationSets because their arities differ and arity
is observable (`len`, unpacking, indexing) — not because they were
allocated at different sites. Then merged containers keep record layout
wherever arities agree, and fall to list layout only where they genuinely
differ, which is the unknown-arity case.

That is exactly what the `PYC_CSDCPA1=2` tuple exclusion was working
around, now shown to apply to `list` as well: neither needs a per-site
contour, both need arity in identity.
