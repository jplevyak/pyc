...
## Repro 2 — fixed, and also not about arity
...
## Repro 2 root cause (2026-08-20, `2756884d`)

Two gaps, both reached by `move = None; while move not in moves:`.

**1. `tuple.__eq__` cannot take a None operand.** The method is
generated by `inject_tuple_compare` and opens with `len(t)`. When `t`
is None that resolves to nothing — and `list.__contains__` compares
EVERY element against the item, so one None item degrades the whole
comparison. The loop condition never becomes true, the body never
runs, and `move` keeps its initial None. That is where the silent
`None` came from.

CPython says a tuple is never equal to a non-tuple, so the generated
`__eq__`/`__lt__` now answer False directly. Giving
`__pyc_None_type__` a `__len__` instead is not an option:
`00_runtime.py` documents why container stubs there inject None into
element types program-wide.

**2. `None == x`, with None as the RECEIVER, had no method at all** —
`unresolved call '__eq__'`. `__pyc_None_type__` defines `__is__`,
`__not__`, `__str__` and friends but never `__eq__`/`__ne__`. Added as
identity, the same answer `__pyc_any_type__.__eq__` gives; safe where a
container stub would not be, because the result is a bool.

Note `(1, 2) == None` appeared to work all along — it was
constant-folded and never exercised the runtime path. The first
measurement that mattered was `None in [(1,2), (3,4)]`.

    (1, 2) == None            False   (was folded, never ran)
    None == (1, 2)            False   (was unresolved)
    None in [(1,2), (3,4)]    False   (was "expression has no type")
    repro 2                   (1, 2)  (was None)

Covered by `tests/tuple_none_compare.py`, both backends.

    C backend      286 passed / 0 failed
    LLVM backend   286 passed / 0 failed
    corpus         70 compiled / 7 failed of 77, per-program identical

## sunfish is NOT fully cleared

`sunfish.py:448` still reports `unresolved call '__not__'`, because its
`gen_moves` is a **generator**, not a list — `move not in
hist[-1].gen_moves()` reaches containment on a generator object rather
than `list.__contains__`, which is a different path from either repro
here. That is the remaining piece of issues/025 item 4 and wants its
own issue rather than more weight on this one.

## Retrospective

Neither repro was an arity problem, which is what this issue is named
after and what its "genuine architectural limit ... per-arity tuple
types are foundational" framing assumed. Repro 1 was a `{tuple, list}`
union created by `tuple.__add__` returning a list; repro 2 was a
missing None case in two comparison methods. Both fixes are library
changes of a few lines. The architectural question the issue raised
