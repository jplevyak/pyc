...
## The plan lives in 129

[129](129-plan-demand-driven-creation-set-splitting.md) is the single
integrated plan; this issue owns no step of it. What this issue contributes
is the **measurement** that motivates step 1 (coarse identity) and the
**negative results** above, which are what keep the plan from re-entering
the dead frame.

The three inert routes — `cselem`, `csshape`, `csmold` — are the one code
question still open here, and it is a judgement call rather than a fix:
delete them per 146's corollary, or keep them as the reproduction of the
`PYC_CSELEM`/`PYC_CSMOLD` experiments this issue cites, which 129 recorded
a decision to do. They now carry a comment at their definitions saying they
answer a question start-merged makes void, so 0 hits is not read as a bug.

If this issue's own measurement is ever re-opened, the checks are:
`IFA_DBG_ELEMTYPE`'s gap closing on chess (95 → nearer 6);
`list.append`'s clone count falling toward the element-type count;
`ess`/`css` NOT growing at the default; `./corpus_sweep.sh -m check`
compared per program; and
