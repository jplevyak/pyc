# 025 — IFA doesn't narrow runtime union types intra-function on conditional branches

**Status:** open, narrower in scope than originally framed. Pyc's only
real "narrowing" is clone-time specialization at function call
boundaries — nothing intra-function. A per-branch AVar-refinement
mechanism was built for this issue and does work for one real pattern
(`is None` on a recursive class-or-None union). A second, textually
similar bug (`isinstance` against a union of *user classes* always
returning `False`) turned out to be a different, unrelated bug and was
fixed separately. **The three originally-described cases (below) all
turn out to be manifestations of
[issues/018](../../issues/018-dict-mixed-key-types-boxing-failure.md)'s
more fundamental gap — a raw scalar/basic-type union has no coherent
runtime representation for *any* polymorphic consumer to dispatch on
— not a narrowing-specific problem.** Even a complete, working
narrowing implementation would not fix Cases 1-3 as they actually
fail; that dependency has moved to 018. Re-verified against current
HEAD 2026-08-11, including this redirect — not just carried over from
old notes.
...
## Pre-FA-inlining workaround (still a latent fragility)
...
or FA→inline→re-FA — not attempted; cost implications unmeasured. This
only matters for the paths that actually reach real narrowing (`is
None` on class unions); Cases 1-3 don't get this far regardless.
...
2. **Branch-correlation recognition** (Case 1's literal original
   ...
   was never built at all; only `isinstance`/`is None`/`is not` are
   recognized today. Worth remembering as a distinct, still-unbuilt
   piece of narrowing infrastructure in case a *class-typed* (not
   scalar) analogue of Case 1 is ever found — 018's gap dominates the
   scalar case regardless of whether this gets built.
3. The isinstance/`__is__` wrapper-recognition workaround
   (pre-FA-inlining) is name-based, not general — relevant only to the
   paths that do reach real narrowing (`is None` on class unions).
...
- Cases 1-3 and the bare-consumption/bare-`print` variants above all
  still fail as described (re-run today) — no `tests/` fixtures exist
  for them; tracked and verified under
...
Real recursive data structures (linked lists, trees) via `is None`
narrowing — already unblocked, composing with 024. Idiomatic
`isinstance`/branch-correlation discriminator patterns over
class-typed unions, if a class-typed analogue of Case 1 is ever found
and item 2 above gets built. The scalar-union cases (Cases 1-3 as
actually filed) are 018's to unblock, not this issue's.
