# issue 056: P_prim_index_object and P_prim_set_index_object (the
# general list branch) cast an index argument straight into
# _CG_norm_idx's int32 parameter with no compatibility check. Once FA
# salvaged an earlier violation to a non-scalar (_CG_any/void)
# index Var, the cast was invalid C, producing a raw clang compile
# error instead of the established runtime-assert salvage convention
# this codebase already uses everywhere else a value can't be
# soundly represented (issue 035's sibling fix, same call sites, the
# stored VALUE rather than the index).
#
# No minimal synthetic repro was ever isolated for this (the original
# filing says so explicitly) -- the trigger needs a receiver AND
# index that both stay genuinely un-monomorphized through to this
# shared list::__getitem__/__setitem__ codegen site, and every
# hand-built attempt at reproducing that shape either got optimized
# away as dead code or never reached the vulnerable clone at all.
# This is the actual vendored program (Apache-2.0, Google Inc. 2011;
# already present in this repo via the shedskin_examples/ git
# subtree, see shedskin_examples/loop/loop.py) that found the bug in
# the first place -- kept verbatim as a compile-only regression test
# (no .exec.check: the bug was specifically pyc emitting C that
...
# identical compiles; reproduces on the unmodified vendored file
# too, confirmed 2026-08-07, not filed as its own issue yet).
# .expect_fail -- the LLVM backend still hard-fails to compile this
# file at all (see ifa/issues' LLVM-parity entry for issue 056).
"""
