...
// -- Globals defined in python_ifa_util.cc (stmtmap, exprmap, sym_*, etc.) --
...
void enter_scope(PycContext &ctx, Sym *in = 0);
...
PycSymbol *find_PycSymbol(PycContext &ctx, cchar *name, int *level = 0, int *type = 0);
...
void scope_sym(PycContext &ctx, Sym *sym, cchar *name = 0);
Sym *make_string(cchar *s);
Sym *make_string(PyObject *o);
void get_syms_args(PycAST *a, arguments_ty args, Vec<Sym *> &has, PycContext &ctx, asdl_seq *decorators = 0);
...
int build_if1_module(mod_ty mod, PycContext &ctx, Code **code);
