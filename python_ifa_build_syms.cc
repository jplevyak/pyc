...
static void import_scope(PycModule *mod, PycCompiler &ctx) {
  void *key = (mod->pymod && !mod->is_builtin) ? (void *)mod->pymod : (void *)mod->mod;
  ctx.imports.add(mod->ctx->saved_scopes.get(key));
}
...
void gen_class_pyda(PyDAST *cdef, PycAST *ast, PycCompiler &ctx) {
